		...
		while ($elkey < $maxel) {
			...
			if (preg_match($tagpattern, $element)) {
				...
				$parentKey = $dom[$key]['parent'];
					...
					// ensure 'thead' exists
					if (!isset($dom[$parentKey]['thead'])) {
						$dom[$parentKey]['thead'] = '';
					}

					if (($dom[$key]['value'] == 'table') AND (!TCPDF_STATIC::empty_string($dom[$parentKey]['thead']))) {
						// remove the nobr attributes from the table header
						$dom[$parentKey]['thead'] = str_replace(' nobr="true"', '', $dom[$parentKey]['thead']);
						$dom[$parentKey]['thead'] .= '</tablehead>';
					}
				...
				if ($dom[$key]['opening']) {
					...
					if (($dom[$key]['value'] == 'td') OR ($dom[$key]['value'] == 'th')) {
						...
						if (!isset($cellspacingx)) {
							$cellspacingx = 0;  // or some default spacing value you want
						}
						...
						if (!isset($cellspacing) || !is_array($cellspacing)) {
							$cellspacing = ['H' => 0, 'V' => 0]; // H = horizontal, V = vertical spacing
						}
